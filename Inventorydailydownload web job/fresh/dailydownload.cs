    ...
    public class dailydownload
    {
        ...
        static void doJob()
        {
          ...
          /* for daily update of record please make  startDateFromToday = 841 and  endDateFromToday = 1 */
            ...
            string[] invoiceList = new string[] { "inventories" };
...

        {
            ...
            mail.Subject = " inventory web job  notification";
                if (result == 1)
                {
        ...
        public static int InsertDataIntoSQLServerUsingSQLBulkCopy(DataTable csvFileData)
        {
            ...
            try
            {
                ...
                using (SqlConnection dbConnection = new SqlConnection(connection1))
                {
                    ...
                    {
                        s.DestinationTableName = "sb_inventory_table";
                        s.BatchSize = 500;
                        ...
                        if (csvFileData.Columns.Contains("reorder_point"))
                        {
                            s.ColumnMappings.Add("reorder_point", "reorder_point");

                        }

                        if (csvFileData.Columns.Contains("minimum_base_quantity"))
                        {
                            s.ColumnMappings.Add("minimum_base_quantity", "minimum_base_quantity");

                        }

                        if (csvFileData.Columns.Contains("batch_id"))
                        {
                            s.ColumnMappings.Add("batch_id", "batch_id");
